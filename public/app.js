...
const prontidaoDe=companyId=>Regras.prontidaoDaEmpresa(acervoDaEmpresa(companyId));

/* Painel do veredito: a resposta objetiva de "posso disputar hoje?". */
function painelProntidao(company,{compacto=false}={}){
  const p=prontidaoDe(company.id);
  ...
  return `<div class="prontidao ${p.status}">
    <div class="pront-head">
      <div><span class="badge ${cor}">${esc(Regras.ROTULO_PRONTIDAO[p.status])}</span>
        ${compacto?'':`<strong>${esc(company.name)}</strong>`}</div>
    ...
    ${problema(p.vencidos,'Vencido','vencido')}
    ${problema(p.vencendo,`Vence em até ${Regras.DIAS_VENCE_LOGO} dias`,'pendente')}
    ${p.status==='apto'?'<p class="pront-ok">Base documental completa e em dia. Confira sempre as exigências específicas do edital.</p>':''}
  </div>`;
}
  ...
  return `<article class="acervo-item ${v.situacao}">
    ...
    ${v.anteriores.length?`<details class="acervo-versoes">
      ...
      ${v.anteriores.map(a=>`<div class="acervo-versao">
        ...
        ${a.path?`<button class="link" data-document="${esc(a.path)}">Abrir</button>`:''}
      </div>`).join('')}
    </details>`:''}
  </article>`;
}
